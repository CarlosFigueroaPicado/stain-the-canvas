(function dashboardModule() {
        ...
        datasets: [
          {
            ...
            data: values,
            borderColor: "#C86B4A",
            backgroundColor: "rgba(200,107,74,0.20)",
            tension: 0.35,
            fill: true,
            pointRadius: 3,
            pointBackgroundColor: "#A34F33"
          }
        ]
      },
      options: {
        ...
        datasets: [
          {
            ...
            data: values,
            backgroundColor: [
              "rgba(200,107,74,0.90)",
              "rgba(200,107,74,0.78)",
              "rgba(200,107,74,0.66)",
              "rgba(200,107,74,0.55)",
              "rgba(200,107,74,0.44)"
            ],
            borderRadius: 8
          }
        ]
  ...
  async function fetchPagedRows(queryFactory, pageSize) {
    ...
    while (true) {
      ...
      if (result.error) {
        throw new Error(result.error.message || "No se pudieron cargar metricas.");
      }
  ...
  async function fetchData() {
    ...
    try {
      const rpcResult = await client.rpc("get_dashboard_metrics", { p_days: 90 });
      if (rpcResult.error) {
    ...
    const [
      ...
      client.from("visitas").select("id", { count: "exact", head: true }),
      client.from("eventos").select("id", { count: "exact", head: true }).eq("tipo", "view_producto"),
      client
        .from("eventos")
      ...
      fetchPagedRows(
        (from, to) =>
          client.from("visitas").select("fecha").order("fecha", { ascending: true }).range(from, to),
        1000
      ),
      fetchPagedRows(
        (from, to) =>
      ...
      client.from(productsTable).select("id,nombre,categoria")
    ]);
    ...
    if (failed && failed.error) {
      throw new Error(failed.error.message || "No se pudieron cargar metricas.");
    }
  ...
  async function cargarDashboard() {
    if (!state.initialized && !initDashboard()) {
      return;
    }
    ...
    try {
      ...
      showStatus(`Error al cargar dashboard: ${error.message}`, "danger");
  ...
  window.dashboardModule = Object.freeze({
    initDashboard,
    cargarDashboard
  });
})();